...
Inputs.text({label: "Name", value: "ExampleChart"})
  ...
  main.variable(observer()).define(["howto","name"], function(howto,name){return(
howto(name, {open: true})
)});
  main.variable(observer()).define(["howto","name"], function(howto,name){return(
howto(name, {open: true, specifier: "@d3/example", imports: {d3: "d3", d3Sankey: "d3-sankey"}})
)});
  main.variable(observer("currentSpecifier")).define("currentSpecifier", function()
{
...
/*
  main.variable(observer("howto")).define("howto", ["currentSpecifier","htl","md","Inputs"], function(currentSpecifier,htl,md,Inputs){return(
...
`}</div>
  ...
  main.variable(observer()).define(["altplot"], function(altplot){return(
altplot(`Plot.barY(alphabet, {x: "letter", y: "frequency"}).plot()`, {open: true})
)});
  main.variable(observer("altplot")).define("altplot", ["htl","md","Inputs"], function(htl,md,Inputs){return(
...
/*
  main.variable(observer("linkplot")).define("linkplot", ["htl","md"], function(htl,md){return(
